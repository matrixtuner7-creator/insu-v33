...
export interface FieldAgent {
  ...
  username?: string;
  password?: string;
  requireLogin?: boolean; // When true, agent portal enforces login; when false, app opens directly
}
...
export interface InsuredPolicy {
  id: string;
  policyNumber: string; // رقم وثيقة التأمين
  insuredName: string; // اسم المؤمن له / العميل
  nationalId: string; // رقم الهوية / السجل المدني
  phone: string; // رقم الهاتف
  plateNumber: string; // رقم لوحة المركبة
  vehicleMake: string; // الشركة المصنعة (تويوتا، هيونداي، مرسيدس...)
  vehicleModel: string; // طراز المركبة (كورولا، توسان...)
  vehicleYear: number; // سنة الصنع
  chassisNumber?: string; // رقم الشاسيه / الهيكل
  insuranceCompany: string; // شركة التأمين
  coverageType: 'شامل' | 'ضد الغير' | 'إلزامي' | 'حريق وسرقة';
  effectiveDate: string; // تاريخ بدء السريان
  expiryDate: string; // تاريخ انتهاء الوثيقة
  policyStatus: 'سارية' | 'منتهية' | 'معلقة' | 'ملغاة';
  coverageLimit: number; // سقف التغطية المالي
  deductible: number; // مبلغ التحمل
...
// ==========================================
// 8-STEP FIELD INVESTIGATION WORKFLOW TYPES
// ==========================================

export type InvestigationStepNumber = 1 | 2 | 3 | 4 | 5 | 6 | 7 | 8;
export type SyncStatusType = 'SAVED_LOCAL' | 'PENDING_SYNC' | 'SYNCED';
...
export interface ArrivalData {
  ...
  lng: number;
  accuracy?: number;
  siteStatus: 'safe' | 'needs_backup';
  backupReason?: string;
  locationAddress?: string;
}

export interface IncidentBasicData {
  caseId: string;
  incidentNumber: string;
  incidentType: string;
  incidentDate: string;
...
export interface CaseParty {
  id: string;
  role: 'insured' | 'third_party' | 'other' | 'witness';
  vehicleRole?: 'INSURED_VEHICLE' | 'THIRD_PARTY_VEHICLE' | 'ADDITIONAL_VEHICLE' | 'WITNESS_VEHICLE';
  roleLabel: string;
  name: string;
  nationalId: string;
  phone: string;
...
export interface DiagramElement {
  id: string;
  type: 'vehicle' | 'road' | 'impact' | 'traffic_light' | 'pedestrian' | 'obstacle' | 'arrow' | 'text' | 'skid_mark' | 'stop_sign' | 'yield_sign' | 'truck' | 'motorcycle' | 'turn_arrow';
  x: number;
  y: number;
  rotation: number;
  label?: string;
...
export interface DiagramDrawingPath {
...
export interface DiagramData {
  roadType: 'straight' | 'intersection' | 'roundabout' | 't_junction' | 'curve' | 'highway';
  elements: DiagramElement[];
  drawingPaths?: DiagramDrawingPath[];
  notes?: string;
...
export interface CaseStatement {
  id: string;
  partyType: 'insured' | 'third_party' | 'witness';
  partyLabel: string;
  personName: string;
  phone: string;
  nationalId?: string;
...
export interface InvestigationSession {
  ...
  currentStep: InvestigationStepNumber;
  ...
  parties: CaseParty[];
  mediaChecklist: CaseMediaItem[];
  diagramData: DiagramData;
  statements: CaseStatement[];
  damageAssessment: DamageAssessmentItem[];
...
export type QrStatus = 'ACTIVE' | 'SUSPENDED' | 'EXPIRED' | 'REVOKED' | 'REPLACED' | 'READ_ONLY';
...
export interface IncidentQrCode {
  id: string;
  caseId: string;
  assignmentId?: string;
  tokenHash: string;
...
export interface QrScanLog {
  ...
  longitude?: number;
  scannedAt: string;
  result: 'SUCCESS' | 'EXPIRED' | 'REVOKED' | 'INVALID' | 'POLICY_SUSPENDED';
  ipAddress?: string;
}

export interface QrSystemSettings {
  enableVehicleQr: boolean;
  enableIncidentQr: boolean;
  allowStickerPrinting: boolean;
  incidentQrExpiryDays: number;
  qrUsageMode: 'OPTIONAL' | 'MANDATORY';
}
